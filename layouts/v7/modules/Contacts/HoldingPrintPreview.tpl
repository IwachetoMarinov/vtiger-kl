	...
	<ul style="list-style-type: none;
   		...
   		background-color: #333;">
         	<li style="float:right"><a style="display: block;
      		...
      		background-color: #bea364;" href="index.php?module=Contacts&view=HoldingPrintPreview&record={$RECORD_MODEL->getId()}&PDFDownload=true">Download</a></li>
	</ul>
{/if}
        <div class="printAreaContainer">
			...
			<td  style="height: 28mm;">
                            <img src='layouts/v7/modules/Contacts/resources/gpm-new-logo.png' style="max-height: 100%; float:right;width: 154px;">
				 ...
				 <tr class="no-border">
				    ...
				    {foreach item=HOLDING from=$HOLDINGS}
                                    ...
                                    <tr class="no-border">
                                        ...
                                        <td>
					    {$HOLDING->longDesc} <br> <span style="font-size: smaller;font-style: italic;">{implode(", ",$HOLDING->modiefiedSerials)}</span>
					</td>
					{assign var=CRYPTO value=['MBTC','ETH']}
					{if in_array(strtoupper($HOLDING->metal),$CRYPTO) }
                                        	<td style='vertical-align: top;text-align:right'>{number_format($HOLDING->pureOz,8)}</td>
					{else}
						<td style='vertical-align: top;text-align:right'>{number_format($HOLDING->pureOz,3)}</td>
					{/if}
                                        <td style='vertical-align: top;text-align:right'>{CurrencyField::convertToUserFormat($totalPrice)}</td>
                                    </tr>
				    {/foreach}
                                {/foreach}
                                <tr>
                                    <th colspan="3">TOTAL MARKET VALUE</th>
                                    <td style='text-align:right'><strong>US$ {CurrencyField::convertToUserFormat($grandTotal)}</strong></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
