...
Vtiger_Edit_Js("GPMIntent_Edit_Js", {}, {
	...
	setSpotPrice: function (selectedMetal) {
		...
		thisInstance.getCurruntMetalSpotPrice(selectedMetal).then(function (data) {
	...
	setupMetalOption: function (selectedMetal) {
		...
		if (thisInstance.itemList === false) {
			thisInstance.getAvailableMetals().then(
				function (data) {
	...
	createMetalOption: function (selectedMetal) {
		var thisInstance = this;
		if (thisInstance.metalOption === false || typeof thisInstance.metalOption[selectedMetal] == 'undefined') {
			Object.keys(thisInstance.itemList[selectedMetal]).forEach((item, index) => {
				thisInstance.metalOption[selectedMetal] = thisInstance.metalOption[selectedMetal] + '<option value="' + item + '">' + thisInstance.itemList[selectedMetal][item]['product_name'] + '</option>'
			});
		}
	},
	...
	registerMetalTypeChangeEvent: function () {
		var thisInstance = this;
		jQuery('select[name="gpm_metal_type"]').on('change.select2', function (e) {
	...
	registerOrderTypeChange: function () {
		jQuery('select[name="gpm_order_type"]').on('change.select2', function (e) {
	...
	calculateTheCurrentLineItemFineOz: function (line) {
		...
		selectedItemFineOz = thisInstance.itemList[thisInstance.selectedMetal][selectedItem]['fine_oz']
	...
	calculateTheCurrentLineItemUSD: function (line) {
		...
		currentSpotPrice = (parseFloat(exactSpotPrice) > 0) ? exactSpotPrice : indicativeSpotPrice;
		...
		if (item_pre_disc_usd != 0) {
			itemUSD = itemTotalOz * currentSpotPrice + parseFloat(item_pre_disc_usd);

		} else {
			itemUSD = itemTotalOz * currentSpotPrice * prem_disc;
			line.find('.item_premium_usd').val(itemUSD - (itemTotalOz * currentSpotPrice))
		}
	...
	registerItemChanges: function () {
		var thisInstance = this;
		jQuery('body').on('change', '.item_metal, .item_qty, .item_premium, .item_premium_usd', function (e) {
	...
	registerCurrencyChangeEvent: function () {
		var thisInstance = this;
		jQuery('select[name="package_currency"]').on('change.select2', function (e) {
			...
			if (selectedCurrency == 'USD') {
				...
				selectedCurrencySGDSpotName = selectedCurrency.toLowerCase() + '_sgd';
				selectedCurrencySGDSpot = forexDate[selectedCurrencySGDSpotName];
				selectedCurrencyUSDSpot = selectedCurrencySGDSpot * 1 / sgd_usdspot;
				jQuery('input[name="indicative_fx_spot"]').val(selectedCurrencyUSDSpot.toFixed(4))
			} else {
				sgd_usdspot = forexDate.usd_sgd;
				selectedCurrencySGDSpotName = selectedCurrency.toLowerCase() + '_sgd';
				selectedCurrencySGDSpot = forexDate[selectedCurrencySGDSpotName];
				selectedCurrencyUSDSpot = sgd_usdspot * 1 / selectedCurrencySGDSpot;
				jQuery('input[name="indicative_fx_spot"]').val(selectedCurrencyUSDSpot.toFixed(4))
	...
	registerPackageAmountChange: function () {
		jQuery('input[name="package_price"]').on('keydown keypress keyup blur', function (e) {
			FxSpot = jQuery('input[name="indicative_fx_spot"]').val();
			selectedCurrency = jQuery('select[name="package_currency"]').val();

			if (selectedCurrency == '') {
				return;
			} else if (selectedCurrency == 'SGD' || selectedCurrency == 'CHF' || selectedCurrency == 'HKD') {
				FxSpot = 1 / FxSpot;
			}
			...
			jQuery('input[name="package_price_usd"]').val(calCulatedAmount.toFixed(2));
	...
	registerEditOrCreate: function () {
		...
		if (jQuery('input[name="record"]').val() > 0) {
			...
			thisInstance.itemIndex = jQuery('#item_container').children().length + 1
		}
	...
	calculateForeignValue: function () {
		...
		FxSpot = (parseFloat(jQuery('input[name="fx_spot_price"]').val()) > 0) ? parseFloat(jQuery('input[name="fx_spot_price"]').val()) : parseFloat(jQuery('input[name="indicative_fx_spot"]').val());
		if (parseFloat(FxSpot) == 0) {
			FxSpot = 1;
		}
		selectedCurrency = jQuery('select[name="package_currency"]').val();
		...
		jQuery('input[name="total_foreign_amount"]').val(calCulatedAmount.toFixed(2));
		...
		jQuery('body').on('click keypress', '.item_fineoz', function (e) {
			...
			if (key == 13) {
				...
				app.helper.showAlertBox({
					title: 'Success!',
					message: '<b>For item ' + thisInstance.itemList[thisInstance.selectedMetal][selectedItem]['product_name'] + ', Total ' + newVal + ' is set as FineOz.</b>'
				});
				return false;
			}
		});
