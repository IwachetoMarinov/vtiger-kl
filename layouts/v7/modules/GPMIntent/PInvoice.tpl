    ...
    <div class="printAreaContainer">
        <div class="full-width">
            <table class="print-tbl">
                ...
                {if isset($COMPANY) && $COMPANY && !empty($COMPANY->get('company_gst_no'))}
                ...
                <tr>
                    <td style="font-size: 9pt; height: 168mm; vertical-align: top;">
                        <table class="activity-tbl" style="margin-bottom:5mm">
                            ...
                            {foreach item=PRODUCT key=cnt from=$RELATED_PRODUCTS}
                                ...
                                <tr class="no-border">
                                    ...
                                    <td style='vertical-align: top;'>{$METAL->get('product_name')}</td>
                            ...
                            {if $INTENT->get('package_currency') neq 'USD'}
                                <tr>
                                    <th style="width:75%;" colspan="4">TOTAL INVOICE AMOUNT IN
                                        {$INTENT->get('package_currency')}: 
                                        {if $INTENT->get('package_currency') eq 'EUR'}(EUR/USD{else}(USD/{$INTENT->get('package_currency')}){/if} RATE
                                        {if $INTENT->get('fx_spot_price') eq 0 }
                                            {$INTENT->get('indicative_fx_spot')}
                                        {else}
                                            {$INTENT->get('fx_spot_price')}
                                        {/if})
                                    </th>
                        ...
                        {if isset($SELECTED_BANK) && $SELECTED_BANK}
                            <div>
                               ...
                               {assign var=ROUTING value=$SELECTED_BANK->get('bank_routing_no')}
                                {if !empty($ROUTING) && trim(count_chars(strtolower($ROUTING),3)) == 'x'}
                        ...
                        {else}
                            <div style="font-weight:bold;color:#a00;">No bank account information available.</div>
                        {/if}
                    </td>
                </tr>
